...
contract WarLogic is AccessControlUpgradeable, UUPSUpgradeable, IWarLogic {
    ...
    function placeCastle(uint256 castle_id, int256 _x, int256 _y) public {
        ...
        if (totalCastlesInMap.current() > 0) {
            ...
            require(_x >= max_castle_distance*-1 &&
                    _x <= max_castle_distance &&
                    _y >= max_castle_distance*-1 &&
                    _y <= max_castle_distance, "Cannot place castle too close or too far from other castle");
        }
    ...
    function clearBurnedCastle(uint256 castleId) public {
        require(contract_castle.isOwnerOf(address(0), castleId) == true, "Token must be burned");
